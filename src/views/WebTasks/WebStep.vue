  ...
  <div>
    ...
    <div v-if="!loading">
      <v-col cols="12" class="d-flex align-center breadcrumbs">
        <router-link to="/tareas-web/" class="breadcrumbs__link"><v-icon large class="blue--text text--darken-2">mdi-chevron-left</v-icon> Tareas Web</router-link>
        <router-link :to="'/tareas-web/' + webTask.id" class="breadcrumbs__link"> {{ webTask.title }} </router-link>
...
export default {
  ...
  async created() {
    ...
    let response = await this.$http
      .get(process.env.VUE_APP_API_DOMAIN + "api/web_tasks/" + this.$route.params.web_task_id);

    this.webTask.set(response.data);
    if(this.$route.params.id) {
      response = await this.$http.get(process.env.VUE_APP_API_DOMAIN + 'api/web_steps/' + this.$route.params.id);
  ...
  methods: {
    ...
    move(step) {
      this.$router.push({
        path: "/tareas-web/"+this.webTask.id+"/paso/"+step
      });
    },
    dismiss() {
    ...
    saveStep() {
      this.$refs.observer.validate().then( valid => {
        if(valid) {
          if(this.$route.params.id) {
            this.$http.put(process.env.VUE_APP_API_DOMAIN + 'api/web_steps/update', {
            ...
            this.$http.post(process.env.VUE_APP_API_DOMAIN + 'api/web_steps/store', {
              image: this.action?.id,
              screenshot: this.img_base64,
              label: this.step,
              focus_size: this.hasFocus ? this.focusSize : null,
              ...
              focus_y: this.hasFocus ? this.yPosition : null,
              details: this.details,
              web_task_id: this.$route.params.web_task_id
            }).then((response) => {
              this.$router.push({
                path: "/tareas-web/"+this.webTask.id
              });
            });
          }
        }
