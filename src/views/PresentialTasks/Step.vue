    ...
    <div v-if="!loading">
      <v-col cols="12" class="sticky-top d-flex align-center breadcrumbs">
        <router-link to="/tareas-presenciales/" class="breadcrumbs__link"
          ...
          Tareas</router-link
        >
        <router-link :to="'/tareas-presenciales/' + task.id" class="breadcrumbs__link">
          {{ task.title }}
        </router-link>
...
export default {
  ...
  methods: {
    ...
    move(step) {
      this.$router.push({
        path: "/tareas-presenciales/" + this.task.id + "/paso/" + step
      });
    },
    dismiss() {
      if (this.originalStep) {
          ...
          if (this.$route.params.id) {
            ...
            this.$http
              .post(process.env.VUE_APP_API_DOMAIN + "api/presential_steps/store", {
                images: images,
                label: this.step,
                presential_task_id: this.$route.params.task_id
              })
              .then(response => {
                this.$router.push({
                  path: "/tareas-presenciales/" + this.task.id
                });
              });
          }
        }
